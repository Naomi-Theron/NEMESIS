...
// Owner numbers
const ownerNumber = ["237678687593"];
...
function loadLocalFiles() {
  ...
  // Check for ridzcoder.html
  if (fs.existsSync('./lib/ridzcoder.html')) {
    console.log("✅ ridzcoder.html found in lib folder");
  } else {
    console.log("⚠️ ridzcoder.html not found in lib folder");
  }
...
if (!fs.existsSync(CREDS)) {
  ...
  if (!session.includes("Nemesis~")) {
    console.log("❌ Invalid NEMESIS-MD session format");
    process.exit(1);
  }
  ...
  console.error("❌ Invalid session data:", err.message);
  process.exit(1);
}
...
// Serve HTML page
app.get('/', (req, res) => {
  const htmlPath = path.join(__dirname, 'lib', 'ridzcoder.html');
    ...
    res.send(`
      ...
      <body>
        <div class="container">
          ...
          <p>Owner: ${config.OWNER_NAME || 'RIDZ CODER'}</p>
        </div>
      </body>
      </html>
    `);
...
app.get('/lib/Ridzcoder.html', (req, res) => {
  res.redirect('/');
});
...
async function getProfilePicture(sock, jid) {
  try {
    ...
    return 'https://files.catbox.moe/dynze8.png'; // Default image
  }
}
...
async function connectToWA() {
  ...
  console.log(`🤖 NEMESIS-MD Connecting with prefix: "${prefix}"`);
  ...
  sock.ev.on('connection.update', (update) => {
    ...
    if (connection === 'close') {
      ...
      console.log("✅ NEMESIS-MD Bot connected to WhatsApp!");
      ...
      try {
        ...
        sock.sendMessage(ownerNumber[0] + '237678687593@s.whatsapp.net', {
          image: { url: imageUrl },
          caption: aliveMsg
        }).catch(() => {
